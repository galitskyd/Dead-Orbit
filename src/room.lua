...
-- raycast line of sight between two points
-- returns true if clear (no blocked tiles)
function has_los(x1,y1,x2,y2)
 local dx=x2-x1
 local dy=y2-y1
 local dist=max(sqrt(dx*dx+dy*dy),1)
 -- step in tile_sz/2 increments
 local steps=flr(dist/4)+1
 local sx=dx/steps
 local sy=dy/steps
 for i=1,steps do
  local cx=x1+sx*i
  local cy=y1+sy*i
  if blocked_at(cx,cy) then
   return false
  end
 end
 return true
